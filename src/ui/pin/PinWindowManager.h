...
class PinWindowManager final : public QObject {
    ...
    PinWindow *createPin(const QImage &image, const QRect &sourceGlobalRect);
    ...
    /// Restore the most recently closed pin window.
    /// Returns nullptr when there is no closed-pin history.
    PinWindow *restoreLastClosedPin();
...
private:
    ...
    void rememberClosedPin(PinWindow *pinWindow);

    PinWindow *createPinFromSnapshot(const PinWindowSnapshot &snapshot);

    QList<QPointer<PinWindow> > m_pinWindows;
    QList<PinWindowSnapshot>    m_closedPinHistory;
    QPointer<PinWindow>         m_activePin;
    bool                        m_closingAllPins          = false;
    bool                        m_suppressFocusActivation = false;
    static constexpr int        kMaxClosedPinHistory      = 10;
};
